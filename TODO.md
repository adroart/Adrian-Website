...
- [ ] **Ship `main` to live and fix the broken auto-deploy.** _(band: you-required)_ `main` is verified green (clean typecheck, 864/864 tests, successful build) but is not on the live site: Cloudflare CI cannot build `main`, so pushes no longer deploy. Delete the malformed Pages secret that is breaking the build, retry the build, and confirm the site updates; until CI is green, deploy with a direct upload (`npm run build` then `wrangler pages deploy dist --project-name adrian-website --branch main`). The remaining activation steps (Google Drive sync, the missing `STRIPE_WEBHOOK_SECRET`, and the proof-before-engraving gate) also stay open. → Detail: [registry-activation-checklist.md](docs/registry-activation-checklist.md)
- [ ] **Walk the collector click-through and settle the open wording.** _(band: you-required)_ The collector journey is wired to the live registry end to end and the registration ceremony, Piece Records, continuity archive, and Successor's Handbook are built, all dark behind `livingLegacy`. Done when every box in the checklist is walked, the eight wording items are settled, and you say the flag can flip. → Gate: [collector-click-through-checklist.md](docs/collector-click-through-checklist.md) · Master: [collector-screen-wording.md](todo/plans/collector-screen-wording.md)
- [ ] **Collector journey later passes.** _(band: agent-runnable)_ Still demo-only by design: the passing walk beyond the contested state, household and heirs, gift wishes from givers, letters, the Resonant Grid room, What was paid, region-grain widening, the Your links backend, and video capture UI (storage layer is built). → Master: [collector-screen-wording.md](todo/plans/collector-screen-wording.md) · Chart: [collector-flow-chart.html](todo/plans/collector-flow-chart.html)
