 ...
 * one-time-code, email/password, and Google OAuth all live in this same Better
 * Auth instance. Replaces Clerk, see todo/plans/better-auth-migration.md and
 * the identity direction in i64os/substrate/directions/identity.md.
 *
...
export function createAuth(env, waitUntil) {
  ...
  return betterAuth({
    ...
    // Create-account / log-in with a password. Better Auth v1.6 does not
    // require email verification unless requireEmailVerification is enabled.
    emailAndPassword: {
      enabled: true,
    },
    // "Continue with Google", only active when the key pair is set in env.
    // Better Auth derives the callback from baseURL + basePath:
