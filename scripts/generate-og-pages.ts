...
for (let num = 1; num <= 64; num++) {
  ...
  const description = `A reading from the Universal Language Oracle has been shared with you. Enter Code ${num} and receive what it holds.`;
  ...
  writeFileSync(join(ulDir, `${num}.html`), html, 'utf-8');
  count++;
}
