            ...
            <div className="max-w-7xl mx-auto w-full px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 gap-16">
                ...
                <div className="space-y-6">
                    <div className="w-full bg-wood-100 border border-wood-200">
                        <img src={art.coverImage} className="w-full h-auto object-cover" alt={`${art.title} by Adrian Rasmussen, ${art.material || 'mixed media'}`} loading="lazy" />
                    </div>
                    {art.images.length > 0 && (
                        <div className="grid grid-cols-3 gap-4">
                            {art.images.map((img, i) => (
                                <img key={i} src={img} className="w-full h-24 object-cover border border-wood-200" alt={`${art.title} detail ${i + 1} by Adrian Rasmussen`} loading="lazy" />
                            ))}
                        </div>
                    )}
                </div>
