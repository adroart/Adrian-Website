...
- **Owner: provision account admin variables** from
  `docs/account-admin-runbook.md`, including `ADMIN_EMAILS` and a separate
  `REGISTRY_STEP_UP_SECRET`.
- **Owner: unset** `CLERK_SECRET_KEY` and `CLERK_WEBHOOK_SECRET` in Cloudflare Pages.
- Locally smoke-test account admin auth after merge: `npm run dev:full`, sign in
  through `/account`, then confirm `/api/admin/verify` returns the allowlisted
  verified identity. Confirm registry unlock and explicit lock separately.
