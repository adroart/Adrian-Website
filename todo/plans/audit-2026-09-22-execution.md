...
## Onboarding refusal and sequencing correction

The current gathering previously started birth and privacy writes concurrently and advanced to Light 47 even when the server refused either write. It now reads actual private state, waits for any birth save, and then submits one combined privacy request. HTTP refusals, pending results and network failures retain the account-owned draft in the existing retry surface. Retrying privacy does not create another birth revision. Failed private reads cannot be treated as missing data or authorize an overwrite; existing person-sharing choices hydrate the controls. An account change invalidates dependent writes and navigation.

Focused builder evidence: 10 helper tests passed, plus 3 mounted browser cases covering held birth ordering, privacy refusal and retry without duplicate birth save, failed read with zero writes, and an actual auth-store account switch during the held save. The same held-birth browser test failed before the fix because two privacy writes had already begun. Frozen-commit verification is reported separately.

The skipped-birth product decision remains pending with Adrian in the coordinator task. Birth remains optional, public-sharing defaults and adult eligibility are unchanged. The current pending state conservatively prevents false completion; it is not an approved final decision between private completion and paused sharing. No production data, provider delivery, publication or deployment occurred.
